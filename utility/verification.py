...
class Verification:
    """ A helper class which offers various static and class-based
    ...
    def verify_chain(cls, blockchain):
        """ Verify the current blockchain and return True if its valid,
        False otherwise. """
        for (index, block) in enumerate(blockchain):
            if index == 0:
                # Genesis block cannot be modified since the hash is confirmed
                # in the first block
                continue
            if block.previous_hash != hash_block(blockchain[index - 1]):
                return False
            # Use the transactions except the reward transaction
            # by specifying :-1
    ...
    def verify_transactions(cls, open_transactions, get_balance):
        ...
        # all function used on Boolean verify_transaction(tx) so all tx must
        # be verified as valid in order to return True
