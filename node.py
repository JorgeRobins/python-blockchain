...
def broadcast_transaction():
    ...
    if not all(key in values for key in required):
        ...
        return jsonify(response), 400
    success = blockchain.add_transaction(values['sender'],
                                         values['recipient'],
                                         values['signature'],
                                         values['amount'],
                                         is_receiving=True)
...
def broadcast_block():
    ...
    if block['index'] == blockchain.chain[-1].index + 1:
        ...
        response = {
            'message': 'Blockchain appears to be shorter. Block not added'
        }
        return jsonify(response), 409
...
def add_transaction():
    ...
    success = blockchain.add_transaction(wallet.public_key,
                                         recipient,
                                         signature,
                                         amount)
    if success:
...
def mine():
    if blockchain.resolve_conflicts:
        response = {
            'message': 'Resolve conflicts first, block not addded!'
        }
        return jsonify(response), 409
